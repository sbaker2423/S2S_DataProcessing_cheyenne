...
tmp2m_1.14$error = tmp2m_1.14$var.degC.cfs - tmp2m_1.14$var.degC.nld
# 2-3 week avg
tmp2m_8.21 = left_join(cfs_tm.anom_8.21, nld_tm.anom, by = c("hru","date", "doy"), suffix = c('.cfs', '.nld')) 
tmp2m_8.21$error = tmp2m_8.21$var.degC.cfs - tmp2m_8.21$var.degC.nld
# 3-4 week avg
tmp2m_15.28 = left_join(cfs_tm.anom_15.28, nld_tm.anom, by = c("hru","date", "doy"), suffix = c('.cfs', '.nld')) 
tmp2m_15.28$error = tmp2m_15.28$var.degC.cfs - tmp2m_15.28$var.degC.nld
...
prate_1.14$error = prate_1.14$var.mmPerD.cfs - prate_1.14$var.mmPerD.nld
# 2-3 week avg
prate_8.21 = left_join(cfs_pr.anom_8.21, nld_pr.anom, by = c("hru","date", "doy"), suffix = c('.cfs', '.nld')) 
prate_8.21$error = prate_8.21$var.mmPerD.cfs - prate_8.21$var.mmPerD.nld
# 3-4 week avg
prate_15.28 = left_join(cfs_pr.anom_15.28, nld_pr.anom, by = c("hru","date", "doy"), suffix = c('.cfs', '.nld')) 
prate_15.28$error = prate_15.28$var.mmPerD.cfs - prate_15.28$var.mmPerD.nld
# list of differenet time periods
prate_list = list(prate_1.14, prate_8.21, prate_15.28)

## calculate percent bias and correlation
tmp2m_seasonal = matrix(as.numeric(0), nrow = length(hru_vec) * 12, ncol = 17)
prate_seasonal = matrix(as.numeric(0), nrow = length(hru_vec) * 12, ncol = 17)

doy_vec = matrix( c(1, 1, 90,
                    2, 32, 120,
                    3, 60, 151, 
                    4, 91, 181, 
                    5, 121, 212,
...
for (j in 1:3) {
  ...
  for (i in 1:length(hru_vec)) {
    hru_prate_df = dplyr::filter(prate_df,hru == hru_vec[i])
    hru_tmp2m_df = dplyr::filter(tmp2m_df,hru == hru_vec[i])
    
    for (k in 1:12) {
      # average accounting for beginning and end date wrap 1999 - 2010
      if (k <= 10 ) {
        hru_prate = filter(hru_prate_df, doy >= doy_vec[k,2]  & doy <= doy_vec[k,3])
        hru_tmp2m = filter(hru_tmp2m_df, doy >= doy_vec[k,2]  & doy <= doy_vec[k,3])
      } else {
        hru_prate = filter(hru_prate_df, doy >= doy_vec[k,2] | doy <= doy_vec[k,3])
        hru_tmp2m = filter(hru_tmp2m_df, doy >= doy_vec[k,2] | doy <= doy_vec[k,3])
      }
      ...
      prate_seasonal[(i + length(hru_vec)*(k - 1)),1] = k
      prate_seasonal[(i + length(hru_vec)*(k - 1)),2] = hru_vec[i]
      prate_seasonal[(i + length(hru_vec)*(k - 1)),(3 +(j-1)*5)] = mean(hru_prate$error)
      prate_seasonal[(i + length(hru_vec)*(k - 1)),(4 +(j-1)*5)] = abs(mean(hru_prate$error))
      prate_seasonal[(i + length(hru_vec)*(k - 1)),(5 +(j-1)*5)] = pbias(hru_prate$var.mmPerD.cfs, hru_prate$var.mmPerD.nld)
      prate_seasonal[(i + length(hru_vec)*(k - 1)),(6 +(j-1)*5)] = cor(hru_prate$var.mmPerD.cfs, hru_prate$var.mmPerD.nld)
      prate_seasonal[(i + length(hru_vec)*(k - 1)),(7 +(j-1)*5)] = cor(hru_prate$anom.mmPerD.cfs, hru_prate$anom.mmPerD.nld)
      
      # calculate tmp2m stats for given time period
      tmp2m_seasonal[(i + length(hru_vec)*(k - 1)),1] = k
      tmp2m_seasonal[(i + length(hru_vec)*(k - 1)),2] = hru_vec[i]
      tmp2m_seasonal[(i + length(hru_vec)*(k - 1)),(3 +(j-1)*5)] = mean(hru_tmp2m$error)
      tmp2m_seasonal[(i + length(hru_vec)*(k - 1)),(4 +(j-1)*5)] = abs(mean(hru_tmp2m$error))
      tmp2m_seasonal[(i + length(hru_vec)*(k - 1)),(5 +(j-1)*5)] = pbias(hru_tmp2m$var.degC.cfs, hru_tmp2m$var.degC.nld)
      tmp2m_seasonal[(i + length(hru_vec)*(k - 1)),(6 +(j-1)*5)] = cor(hru_tmp2m$var.degC.cfs, hru_tmp2m$var.degC.nld)
      tmp2m_seasonal[(i + length(hru_vec)*(k - 1)),(7 +(j-1)*5)] = cor(hru_tmp2m$anom.degC.cfs, hru_tmp2m$anom.degC.nld)
    }
  }
}
...
# label columns
colnames(tmp2m_seasonal) <- c('season', 'hru',
                              'meanErr.1-2wk', 'absmeanErr.1-2wk' ,'pbias.1-2wk', 'cor.1-2wk','acc.1-2wk',
...
colnames(prate_seasonal) <- c('season', 'hru',
                           'meanErr.1-2wk', 'absmeanErr.1-2wk' ,'pbias.1-2wk', 'cor.1-2wk','acc.1-2wk',
                           'meanErr.2-3wk', 'absmeanErr.2-3wk' ,'pbias.2-3wk', 'cor.2-3wk','acc.2-3wk',
                           'meanErr.3-4wk', 'absmeanErr.3-4wk' ,'pbias.3-4wk', 'cor.3-4wk','acc.3-4wk')

## Save statistics
saveRDS(tmp2m_seasonal, file = c('tmp2m_analysisSeasonal.rds'))
saveRDS(prate_seasonal, file = c('prate_analysisSeasonal.rds'))
