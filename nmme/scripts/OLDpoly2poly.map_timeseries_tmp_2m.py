...
def writeNetCDFData(fn, tmp_2m, issue_time, fcst_time, hrus):
    ...
    print("dim2size %d" % dim2size)
    print(tmp_2m.shape)

    dim_1 = ncfile.createDimension('issue_time', None)  # time axis - record dimension
    dim_2 = ncfile.createDimension('fcst_time', None)  # 
    dim_3 = ncfile.createDimension('hru', dim2size)  # hru axis

    max_strlen = 12  # EAC
    dim_4 = ncfile.createDimension('strlen', max_strlen) # string length for hrus EAC adde
  
    # Define dimensions variable to hold the var
    t = ncfile.createVariable('fcst_time', 'i4', ('fcst_time', ))
    hru = ncfile.createVariable('hru', 'S1', ('hru', 'strlen'))   # edited EAC
    tn = ncfile.createVariable('issue_time', 'i4', ('issue_time', ))
   ...
   # matn = ncfile.createVariable('tmin','f4',('time','hru',))
    tavg_2m = ncfile.createVariable('tmp2m','f4',('issue_time','fcst_time','hru',))  

   
    # Populate netcdf file variables
    tn[:]     = issue_time
    t[:]      = fcst_time   # NM added
...
def compAvgVal(nc_wgt,nc_in,varname):
    ...
    timeVal = getNetCDFData(nc_in,'issue_time')   # could speed this up by not importing this after the first time
    fcstVal = getNetCDFData(nc_in,'fcst_time') #forecast time
    print("fcst time %s" % fcstVal)

    issue_t = dataVal.shape[0]  # trace number
    dim1size = dataVal.shape[1]  # time dimension
    gridrows = dataVal.shape[2]  # grid rows dim (y)
    gridcols = dataVal.shape[3]  # grid cols dim (x)
    print("times %d rows %d cols %d issue_time %d" % (dim1size, gridrows, gridcols, issue_t))
    ...
    for h in range(len(hruIDs)):
  	...
  	print("j_index = %s" % j_index)
	print("i_index = %s" % i_index)
	print("gridrows = %d" % gridrows)
	print("gridcols = %d" % gridcols)
	print("time = %d" % dim1size)
        ...
        for c in range(0,ncells):
            ...
            # weight values
            Val1=wgtval[c]*dataVal[:,:,yj,xi].reshape(-1,1,1)  # this is a vector with time
        ...
        Val2=oriVal.sum(axis=1).reshape(-1,1,1)   # sum up weighted HRU cell
        del oriVal
	print(Val2.shape) 
    ...
    return timeVal,fcstVal,hruIDs,wgtedVal
...
if __name__ == '__main__':
    ...
    if len(args) == 3:
       ...
       # times,hrus,MATRng=compAvgVal(nc_wgt,nc_in,'t_range')
        times,issue_time,hrus,T2M=compAvgVal(nc_wgt,nc_in,'tmp2m')
        writeNetCDFData(nc_out, T2M, times, issue_time, hrus)      
