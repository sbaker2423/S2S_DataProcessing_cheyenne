...
def writeNetCDFData(fn, tmp2m, times, hrus):
    ...
    dim2size = tmp2m.shape[1]
   
    dim_1 = ncfile.createDimension('fcst_time', None)  # time axis - record dimension
   ...
   # matn = ncfile.createVariable('tmin','f4',('time','hru',))
    tavg_2m = ncfile.createVariable('tmp2m','f4',('fcst_time','hru',))  
   ...
   # matn[:,:] = (matavg - matrng/2)
    ...
    tavg_2m.associate = 'fcst_time hru'  
    tavg_2m.units = 'K'
...
def compAvgVal(nc_wgt,nc_in,varname):
    ...
    for h in range(len(hruIDs)):
        ...
        for c in range(0,ncells):
            ...
            # weight values
            Val1=wgtval[c]*dataVal[0,:,yj,xi].reshape(-1,1)  # this is a vector with time
